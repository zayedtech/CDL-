...
sync syncdp(.clk(clk), .n_rst(n_rst), .async_in(dp_in), .sync_out(dp));
...
timer timerdp(.clk(clk), .n_rst(n_rst), .enable_timer(1'b1), .data_size(3), .bit_period(cycles),  .shift_strobe(dpshift_strobe), .packet_done(end_packet));
...
timer timerfsm16(.clk(clk), .n_rst(n_rst), .enable_timer(timer_16), .data_size(16), .bit_period(14'b00000000001000),  .shift_strobe(unused1), .packet_done(strobes_16));
...
timer timerpidsync(.clk(clk), .n_rst(n_rst), .enable_timer(en_timer), .data_size(16), .bit_period(14'b00000000001000),  .shift_strobe(pidsyncshift_strobe), .packet_done(syncpid_end));
...
timer timerdata(.clk(clk), .n_rst(n_rst), .enable_timer(flush_and_start), .data_size(8), .bit_period(14'b00000000001000),  .shift_strobe(data_strobe), .packet_done(data_end));
